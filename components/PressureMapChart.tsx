...
const PressureMapChart: React.FC<PressureMapChartProps> = ({ data }) => {
    
    const CustomTooltip: React.FC<any> = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            return (
                <div className="bg-slate-700/80 backdrop-blur-sm p-3 border border-slate-600 rounded-lg shadow-lg">
                    <p className="label text-slate-300">{`Distancia: ${label.toFixed(2)} m`}</p>
                    <p className="intro text-sky-400">{`Caída de Presión: ${formatYAxis(payload[0].value)}`}</p>
                </div>
            );
        }
        return null;
    ...
    const formatYAxis = (value: number): string => {
        ...
        if (absValue >= 1e5) {
            return `${(value / 1e5).toFixed(2)} bar`;
        }
        if (absValue >= 1e3) {
            ...
            <ResponsiveContainer width="100%" height="100%">
                ...
                >
                    ...
                    <XAxis 
                        ...
                        label={{ value: 'Longitud (m)', position: 'insideBottom', offset: -15, fill: '#94a3b8' }}
                        stroke="#94a3b8"
                        tick={{ fill: '#cbd5e1', fontSize: 12 }}
                    />
                    <YAxis 
                        label={{ value: 'Caída de Presión', angle: -90, position: 'insideLeft', fill: '#94a3b8', dx: -40 }}
                        stroke="#94a3b8"
                        tick={{ fill: '#cbd5e1', fontSize: 12 }}
